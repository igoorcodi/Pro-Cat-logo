    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col xl:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={onAdd}
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-indigo-100"
          >
          ...
          {filteredCustomers.map(customer => (
            ...
            >
              ...
              <div className="flex items-center gap-5 mb-8">
                <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center font-black text-2xl shadow-inner uppercase">
                  {customer.name[0]}
                </div>
                <div>
                  <h4 className="text-xl font-black text-slate-800 tracking-tight leading-tight">{customer.name}</h4>
              ...
              <div className="space-y-3 mb-8">
                <div className="flex items-center gap-3 text-slate-500 text-sm font-medium">
                  <Mail size={16} className="text-indigo-400" /> {customer.email || 'Sem email'}
                </div>
                <div className="flex items-center gap-3 text-slate-500 text-sm font-medium">
                  <Phone size={16} className="text-indigo-400" /> {customer.phone}
                </div>
                {customer.city && (
                  <div className="flex items-center gap-3 text-slate-500 text-sm font-medium">
                    <MapPin size={16} className="text-indigo-400" /> {customer.city}, {customer.state}
                  </div>
                )}
              </div>
      ...
      {filteredCustomers.length === 0 && (
        <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
          ...
          <p className="text-sm text-slate-400 font-bold mt-2">Comece cadastrando seu primeiro cliente parceiro.</p>
        </div>
      )}
    </div>
  );
