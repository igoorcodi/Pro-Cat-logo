...
import { Catalog, Product, Company, Category, Subcategory, CartItem, Promotion } from '../types';
import { 
  ShoppingCart, 
  MessageCircle, 
  Search, 
...
const PublicCatalogView: React.FC<PublicCatalogViewProps> = ({ catalog, products, seller, company, isLoading, error, categories = [], promotions = [], onBack }) => {
  ...
  const discountValue = useMemo(() => {
    ...
    if (appliedPromotion.discount_type === 'fixed') {
      ...
      // Regra de Teto Máximo de Desconto
  ...
  // Validar se o cupom aplicado ainda é válido ao mudar o carrinho
  useEffect(() => {
    if (appliedPromotion && appliedPromotion.min_order_value > cartSubtotal) {
      setAppliedPromotion(null);
      setCouponError(`Cupom removido: valor mínimo de R$ ${appliedPromotion.min_order_value.toLocaleString('pt-BR')} não atingido.`);
    }
  }, [cartSubtotal, appliedPromotion]);

  const handleApplyCoupon = () => {
    setCouponError(null);
    const promo = promotions.find(p => p.code.toUpperCase() === couponInput.toUpperCase() && p.status === 'active');
    ...
    if (promo.min_order_value > cartSubtotal) {
      setCouponError(`Valor mínimo para este cupom é R$ ${promo.min_order_value.toLocaleString('pt-BR')}`);
      return;
    }
  ...
  const nextImage = (e?: React.MouseEvent) => { e?.stopPropagation(); if (!viewingProduct?.images) return; setCurrentImageIndex((prev) => (prev + 1) % viewingProduct.images!.length); };
  const prevImage = (e?: React.MouseEvent) => { e?.stopPropagation(); if (!viewingProduct?.images) return; setCurrentImageIndex((prev) => (prev - 1 + viewingProduct.images!.length) % viewingProduct.images!.length); };
  ...
  return (
    <div className="bg-slate-50 min-h-screen selection:bg-indigo-100 flex flex-col relative overflow-x-hidden">
      ...
      <div className="shrink-0 bg-white border-b border-slate-100">
        ...
        {currentSubcategories.length > 0 && (
          <div className="max-w-7xl mx-auto px-4 pb-4 overflow-x-auto no-scrollbar flex items-center gap-2 animate-in slide-in-from-top-2">
            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mr-2"><Filter size={12} /> Filtro:</div>
            <button onClick={() => setSelectedSubcategoryId('Todas')} className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all ${selectedSubcategoryId === 'Todas' ? 'bg-indigo-50' : 'bg-white border border-slate-200 text-slate-400'}`} style={selectedSubcategoryId === 'Todas' ? { backgroundColor: primaryColor + '15', color: primaryColor } : {}}>Todas</button>
            {currentSubcategories.map(sub => (
              <button key={sub.id} onClick={() => setSelectedSubcategoryId(sub.id)} className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all ${selectedSubcategoryId === sub.id ? '' : 'bg-white border border-slate-200 text-slate-400'}`} style={selectedSubcategoryId === sub.id ? { backgroundColor: primaryColor + '15', color: primaryColor } : {}}>{sub.name}</button>
            ))}
          </div>
        )}
      </div>
              ...
              <div className="aspect-square bg-slate-50 overflow-hidden relative shrink-0">
                {product.images && product.images.length > 0 ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-2 p-4">
                    ...
                    <span className="text-xs font-black uppercase tracking-tighter text-center">Sem foto</span>
                  </div>
                )}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              {displayLogoUrl ? ( <img src={displayLogoUrl} className="w-10 h-10 rounded-xl object-cover shadow-sm" alt="Logo" /> ) : ( <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: primaryColor }}> <Building2 size={24} /> </div> )}
              <h4 className="font-black text-slate-900 uppercase tracking-tighter">{company?.trading_name || company?.name || catalog?.name}</h4>
            </div>
            <p className="text-sm text-slate-500 leading-relaxed font-medium">Sua vitrine digital profissional. Confira nossos produtos e entre em contato para fazer seu pedido diretamente pelo WhatsApp.</p>
          </div>

          <div className="space-y-4">
            <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Canais de Contato</h5>
            <ul className="space-y-3">
              {company?.whatsapp && ( <li> <a href={`https://wa.me/${company.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-slate-600 hover:brightness-90 transition-colors group"> <div className="w-8 h-8 rounded-lg flex items-center justify-center group-hover:brightness-110 transition-all" style={{ backgroundColor: primaryColor + '10', color: primaryColor }}> <Phone size={16} /> </div> <span className="text-sm font-bold">{company.whatsapp}</span> </a> </li> )}
            </ul>
          </div>

          <div className="space-y-4">
            <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Onde Estamos</h5>
            {company?.address ? ( <div className="flex items-start gap-3 text-slate-600 group"> <div className="w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center shrink-0"> <MapPin size={16} /> </div> <div className="text-sm font-medium leading-tight"> <p className="font-bold">{company.address}, {company.number || 'S/N'}</p> <p className="text-slate-400 text-xs">{company.neighborhood}</p> <p className="text-slate-400 text-xs">{company.city} - {company.state}</p> </div> </div> ) : ( <p className="text-sm text-slate-400 italic">Vendas exclusivamente online.</p> )}
          </div>

          <div className="space-y-4">
            <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Siga-nos</h5>
            <div className="flex gap-3">
              {company?.instagram && ( <a href={`https://instagram.com/${company.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-xl bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-600 text-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform active:scale-95"> <Instagram size={20} /> </a> )}
            </div>
          </div>
        </div>
      </footer>
      ...
      {cart.length > 0 && !isCartOpen && (
        <button onClick={() => setIsCartOpen(true)} className="fixed bottom-6 right-6 z-50 p-5 text-white rounded-full shadow-2xl lg:hidden animate-bounce flex items-center justify-center" style={{ backgroundColor: primaryColor, boxShadow: `0 25px 50px -12px ${primaryColor}40` }}>
          <ShoppingCart size={24} />
          <span className="absolute -top-1 -right-1 w-6 h-6 bg-red-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-lg">{cart.reduce((a, b) => a + b.quantity, 0)}</span>
        </button>
      )}
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {cart.length > 0 ? (
                      ...
                      <div key={item.id} className="flex gap-4 p-4 bg-slate-50 rounded-3xl border border-slate-100 group animate-in slide-in-from-bottom-2">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-2 py-1"><button onClick={() => updateCartQuantity(item.id, -1)} className="p-1 text-slate-400 hover:brightness-75 transition-all" style={{ color: primaryColor }}><Minus size={14}/></button><span className="text-xs font-black w-4 text-center">{item.quantity}</span><button onClick={() => updateCartQuantity(item.id, 1)} className="p-1 text-slate-400 hover:brightness-75 transition-all" style={{ color: primaryColor }}><Plus size={14}/></button></div>
                    ...
                    {appliedPromotion ? (
                      <div className="flex items-center justify-between bg-emerald-50 border border-emerald-100 p-3 rounded-2xl animate-in zoom-in">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-black text-emerald-700 uppercase">{appliedPromotion.code}</p>
                            <p className="text-[9px] font-bold text-emerald-600">
                              Desconto: {appliedPromotion.discount_type === 'percentage' ? `${appliedPromotion.discount_value}%` : `R$ ${appliedPromotion.discount_value.toLocaleString('pt-BR')}`}
                              {appliedPromotion.discount_type === 'percentage' && appliedPromotion.max_discount_value > 0 && ` (Máx R$ ${appliedPromotion.max_discount_value})`}
                            </p>
                          </div>
                        </div>
                        <button onClick={() => setAppliedPromotion(null)} className="p-1.5 text-emerald-400 hover:text-red-500 transition-colors"><X size={18}/></button>
                      </div>
                    ) : (
                      <div className="flex gap-2">
                        <input 
                          ...
                          className="flex-1 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold uppercase outline-none focus:ring-2"
                          style={{ '--tw-ring-color': primaryColor + '30' } as any}
                        />
                        <button 
                          onClick={handleApplyCoupon}
                          disabled={!couponInput.trim()}
                          className="px-4 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 disabled:opacity-50 transition-all"
                        >
                          Aplicar
                        </button>
                      </div>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-40"><ShoppingCart size={64} className="text-slate-300" /><div><p className="font-black text-slate-800 uppercase tracking-widest text-sm">Carrinho Vazio</p><p className="text-xs font-bold text-slate-400 mt-1">Selecione produtos para continuar.</p></div><button onClick={() => setIsCartOpen(false)} className="px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest" style={{ backgroundColor: primaryColor + '10', color: primaryColor }}>Explorar Produtos</button></div>
              )}
            </div>
              ...
              <div className="p-6 bg-slate-900 text-white space-y-6">
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-slate-400">
                    ...
                    <span className="text-sm font-bold">R$ {cartSubtotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                  </div>
                  {appliedPromotion && (
                    <div className="flex items-center justify-between text-emerald-400">
                      <span className="text-[10px] font-black uppercase tracking-widest">Desconto</span>
                      <span className="text-sm font-bold">- R$ {discountValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    <button onClick={() => { try { const url = window.location.href; navigator.clipboard.writeText(url); alert("Link do catálogo (com carrinho) copiado!"); } catch (e) { alert("Erro ao copiar link."); } }} className="w-full py-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all flex items-center justify-center gap-2"><Share2 size={14} /> Compartilhar Carrinho</button>
                </div>
              </div>
            )}
          </div>
      ...
      {viewingProduct && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-4xl rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 duration-500 flex flex-col sm:flex-row max-h-[90vh] sm:max-h-[80vh]">
            <div className="w-full sm:w-1/2 bg-slate-100 relative h-64 sm:h-auto shrink-0 group/carousel">
              {viewingProduct.images && viewingProduct.images.length > 0 ? (
                <>
                  <div className="w-full h-full relative">
                    <img src={viewingProduct.images[currentImageIndex]} className="w-full h-full object-cover transition-opacity duration-300" alt={`${viewingProduct.name} - ${currentImageIndex + 1}`} />
                    {viewingProduct.images.length > 1 && (
                      <><button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white rounded-full transition-all lg:opacity-0 lg:group-hover/carousel:opacity-100 active:scale-90"><ChevronLeft size={24} /></button><button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/20 hover:bg-white/40 backdrop-blur-md text-white rounded-full transition-all lg:opacity-0 lg:group-hover/carousel:opacity-100 active:scale-90"><ChevronRight size={24} /></button><div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">{viewingProduct.images.map((_, idx) => ( <button key={idx} onClick={() => setCurrentImageIndex(idx)} className={`w-2 h-2 rounded-full transition-all ${idx === currentImageIndex ? 'w-6 bg-white' : 'bg-white/40 hover:bg-white/60'}`} /> ))}</div></>
                    )}
                  </div>
                </>
              ) : ( <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-slate-100 gap-3"><ImageOff size={48} strokeWidth={1} /><p className="text-[10px] font-black uppercase tracking-widest">Imagem indisponível</p></div> )}
              <div className="absolute top-4 left-4 z-10"><span className="bg-slate-900/90 backdrop-blur-md text-white text-[12px] font-black uppercase tracking-widest px-4 py-1.5 rounded-xl shadow-2xl border border-white/20">Cód: #{String(viewingProduct.id).substring(0, 8)}</span></div>
              <button onClick={() => setViewingProduct(null)} className="absolute top-4 right-4 p-2 bg-slate-900/40 backdrop-blur-md text-white rounded-full sm:hidden transition-transform active:scale-90 z-10"><X size={24} /></button>
            </div>
            <div className="w-full sm:w-1/2 p-6 sm:p-10 flex flex-col overflow-y-auto">
              ...
              <div className="mb-6"><span className="text-[10px] font-black uppercase tracking-widest" style={{ color: primaryColor }}>{viewingProduct.category || 'Geral'}</span><h2 className="text-xl sm:text-2xl font-black text-slate-800 leading-tight mt-1">{viewingProduct.name || 'Sem nome'}</h2></div>
              <div className="flex-1 space-y-6"><div><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Sobre este item</h4><p className="text-slate-600 text-sm leading-relaxed">{viewingProduct.description || 'Nenhuma descrição fornecida.'}</p></div>
                {viewingProductSubs.length > 0 && (
                  <div className="animate-in slide-in-from-top-2 duration-500"><div className="flex items-center justify-between mb-3"><h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">Escolha uma opção</h4>{viewingProductSubs.length > 0 && !selectedSubForOrder && ( <span className="text-[9px] font-black text-amber-500 uppercase tracking-tight animate-pulse">Seleção necessária</span> )}</div><div className="flex flex-wrap gap-2">{viewingProductSubs.map(sub => { const isSelected = selectedSubForOrder?.id === sub.id; return ( <button key={sub.id} onClick={() => setSelectedSubForOrder(sub)} className={`flex items-center gap-2 px-4 py-3 rounded-xl text-xs font-bold transition-all border-2 ${ isSelected ? 'text-white shadow-md scale-105' : 'bg-slate-50 border-slate-100 text-slate-500 hover:brightness-95 active:scale-95' }`} style={isSelected ? { backgroundColor: primaryColor, borderColor: primaryColor, boxShadow: `0 10px 15px -3px ${primaryColor}20` } : {}} > {isSelected && <Check size={14} className="stroke-[4]" />} {sub.name} </button> ); })}</div></div>
                )}
              </div>
              <div className="mt-auto pt-6 border-t border-slate-100 flex flex-col gap-4"><div className="flex items-baseline justify-between"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Preço Unitário</p><span className="text-2xl sm:text-3xl font-black text-slate-900">R$ {(viewingProduct.price || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div><button onClick={() => addToCart(viewingProduct, selectedSubForOrder)} disabled={viewingProductSubs.length > 0 && !selectedSubForOrder} className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl transition-all active:scale-95 ${ viewingProductSubs.length > 0 && !selectedSubForOrder ? 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none' : 'text-white hover:brightness-110' }`} style={!(viewingProductSubs.length > 0 && !selectedSubForOrder) ? { backgroundColor: primaryColor, boxShadow: `0 10px 20px -5px ${primaryColor}40` } : {}} ><Plus size={20} strokeWidth={3} /> Adicionar ao Carrinho</button></div>
            </div>
          </div>
        </div>
      )}
